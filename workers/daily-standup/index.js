 ...
 *
 * Setup:
 * 1. Create a .env file with GEMINI_API_KEY and ZOHO_WEBHOOK_URL; copy config/examples/repos.example.txt to repos.txt
 * 2. Optional: standup-notes.txt — lines starting with "-" for non-git work (cleared after a successful post)
 * 3. Run: npm start  (or node workers/daily-standup/index.js)
 */
...
function resolveRepoPathsFile() {
  return resolveEnvPath(process.env.REPO_PATHS_FILE, "repos.txt");
}
...
const ERROR_LOG_FILE = resolveEnvPath(process.env.ERROR_LOG_FILE, "standup-errors.txt");
...
const LAST_RUN_FILE = resolveEnvPath(process.env.LAST_RUN_FILE, ".last-run");
 ...
 */
const MANUAL_NOTES_FILE = resolveEnvPath(process.env.MANUAL_NOTES_FILE, "standup-notes.txt");
 ...
 * @returns {string[]}
 */
const STANDUP_NOTES_EXAMPLE_FILE = join(CONFIG_EXAMPLES_DIR, "standup-notes.example.txt");
...
function warnIfNotesOnlyInExampleFile() {
  ...
  try {
    ...
    if (exampleNotes.length > 0) {
      console.warn(
        `⚠️  Found ${exampleNotes.length} bullet line(s) in config/examples/standup-notes.example.txt, but the bot reads standup-notes.txt only.`,
      );
      console.warn(`   Copy or rename: cp config/examples/standup-notes.example.txt standup-notes.txt\n`);
    }
  } catch {
    // ignore
  }
...
async function main() {
  ...
  if (REPO_PATHS.length === 0) {
    const message =
      "No repositories configured. Copy config/examples/repos.example.txt to repos.txt and add your local git paths.";
